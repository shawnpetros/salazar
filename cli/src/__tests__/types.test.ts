...
describe("F005 — SessionRecord type", () => {
  ...
  it("has a specName field", () => {
    expect(record.specName).toBe("my-spec.md");
  });

  it("has a featuresTotal field", () => {
    expect(record.featuresTotal).toBe(21);
  });

  it("has a featuresPassing field", () => {
    expect(record.featuresPassing).toBe(17);
  });

  it("has a score field", () => {
    expect(record.score).toBe(8.1);
  });

  it("has a durationMs field", () => {
    expect(record.durationMs).toBe(4200000);
  });

  it("has a cost field", () => {
    expect(record.cost).toBe(2.35);
  });

  it("has a timestamp field", () => {
    expect(record.timestamp).toBe("2026-03-27T12:00:00.000Z");
  });
  ...
  it("accepts zero values for numeric fields", () => {
    ...
    expect(empty.featuresTotal).toBe(0);
    expect(empty.featuresPassing).toBe(0);
    expect(empty.score).toBe(0);
    expect(empty.durationMs).toBe(0);
    expect(empty.cost).toBe(0);
  });

  it("featuresPassing can equal featuresTotal (perfect run)", () => {
    const perfect: SessionRecord = { ...record, featuresPassing: 21 };
    expect(perfect.featuresPassing).toBe(perfect.featuresTotal);
  });
